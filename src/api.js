$(document).ready(() => {
  ...
  const getGifs = async (query, limit) => {
    try {
      await fetch(`http://api.giphy.com/v1/gifs/search?q=${query}&api_key=${config.API_KEY}&limit=${limit}`)
  ...
  const displayDirector = (response, status) => {
    ...
    switch (status) {
      case 200:
        ...
        break;
      case 403:
        showError("Oops! Looks like you are missing the API Key &#128533");
        break;
      case 404:
        showError("Sorry. We couldn't find what you are looking for. Perhaps try searching for something else? &#129488");
        break;
      default:
        showError("We are not sure what happened. But our server is not responding. &#128528");
    }
    return response;
  };
  ...
  const showResult = (data) => {
    ...
    data.map((item) => {
      $(".result-container-c").append(`<div class="result-item-c" id=${item.id}><img src=${item.images.preview_gif.url} alt=${item.title}/></div>`);
    });
